...
:-include('../Philosophy/find_details.pl').
...
append(Grad_students_aa1,[[A,B,Student_number,TS_of_enrollment,Year_of_enrollment,Month_of_enrollment,Day_of_enrollment,TS,Year_of_graduation,Month_of_graduation,Day_of_graduation,Course,Course_type,Years_to_complete,Essays_left,As]],Grad_students_aa3)
	...
	(true%toss_coin 
	-> (get_texts(Course,Texts_1),
	
	findall(Texts_2,member([_Path1,_File1,Texts_2],Texts_1),Texts),
	...
	**/
	...
	working_directory(_, '../Lucian-Academy/fddb/')
		
	findall([DB,WN],(

	member([Path1,File1,_Texts_2],Texts),
	
	catch(working_directory(_, Path1), _, (concat_list(["mkdir ",Path1],Command),shell1_s(Command),working_directory(_, Path1))),

	concat_list(["find_details_db_",File],File2),

	catch((exists_file(File2),
	load_db(Dept,File2,[string,_],WN,DB)),
	_, 
	(find_db(Dept,File,argument,[DB,WN])))
	)
	,Paths_files),
...
get_texts(Dept,Texts) :-
	...
	findall([Dept,Filex1,String02b],(member(Filex1,G),
